            ...
            for (int curInput = 0; curInput < Math.Min(inputs.Length, GetLengthOfInputVector()); curInput++)
            {
                if (curInput + 1 <= modelParameters.LinearGains.Length)
                {
                    double curUvalue = inputs[curInput];
                      if (lastGoodValuesOfInputs == null || lastGoodValuesOfInputs.Length <= curInput) { var oldArr = lastGoodValuesOfInputs; lastGoodValuesOfInputs = new double[inputs.Length]; if (oldArr != null) Array.Copy(oldArr, lastGoodValuesOfInputs, Math.Min(oldArr.Length, inputs.Length)); }
                    ...
                    {
                        if (lastGoodValuesOfInputs == null || lastGoodValuesOfInputs.Length <= curInput)
                        {
                            var oldArr = lastGoodValuesOfInputs;
                            lastGoodValuesOfInputs = new double[inputs.Length];
                            if (oldArr != null) Array.Copy(oldArr, lastGoodValuesOfInputs, Math.Min(oldArr.Length, inputs.Length));
                        }
