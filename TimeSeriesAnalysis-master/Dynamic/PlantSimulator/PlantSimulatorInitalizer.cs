...
namespace TimeSeriesAnalysis.Dynamic
{
    ...
    public class PlantSimulatorInitalizer
    {
            ...
            Dictionary<string,List<string>> compLoopDict)
        {
            ...
            if (!isOk)
            {
                System.Console.WriteLine("Failed at ForwardCalcNonPID.");
                isOk = false;
            }
            ...
            {
                System.Console.WriteLine("Failed at SetPidControlledVariablesToSetpoints.");
                isOk = false;
            }
            ...
            if (!isOk)
            {
                System.Console.WriteLine("Failed at BackwardsCalcFeedbackLoops.");
                isOk = false;
            }
            ...
            ref Dictionary<string, double> signalValuesAtT0)
        {
            ...
            foreach (var pidID in pidIDs)
            {
               ...
               /* bool doV1 = false;
                ...
                {
                    // Check if process model is compatible (Linear Unit Model)
                    if (!(simulator.modelDict[processId] is UnitModel))
                        continue;
            ...
            ref List<string> uninitalizedPID_IDs)
        {
            ...
            for (int subSystem = orderedSimulatorIDs.Count - 1; subSystem > 0; subSystem--)
            {
                ...
                else if (uFreeIndices.Length == 1)
                {
                    ...
                    {
                        Shared.GetParserObj().AddWarning($"PlantSimulatorInitalizer:something went wrong when calculating steady-state inputs during init. Model: {model.GetID()} Type: {model.GetType().Name}. Skipping this model instead of failing the whole thing.");
                    }
                }
            }
            return true;
