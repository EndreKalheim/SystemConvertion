...
namespace TimeSeriesAnalysis.Dynamic
{
    ...
    public class ConnectionParser
    {
            ...
            Dictionary<string, ISimulatableModel> modelDict)
        {
            ...
            if (unprocessedModels.Count() > 0)
            {
                Console.WriteLine($"[DEBUG] Unprocessed Models left: {string.Join(", ", unprocessedModels)}");
                Shared.GetParserObj().AddError("ConnectionParser.DetermineCalculationOrderOfModels() did not parse all models. Forcing unresolved models into execution schedule.");
                foreach (string unresolvedModel in unprocessedModels)
                {
                    orderedModelAndLoopIDs.Add(unresolvedModel);
                }
            }
