...
namespace TimeSeriesAnalysis.Dynamic
{
    ...
    public class PlantSimulator
    {
                ...
                foreach (var inputId in inputIDs)
                {
                    ...
                    if (simDataSet.ContainsSignal(inputId))
                    {
                        ...
                        if (retVal != null)
                        {
                            ...
                            {
                                // Fix algebraic loops by falling back to t-1 if t is missing
                                if (timeIndexInternal > 0)
                                {
                                    var prev = simDataSet.GetValue(inputId, timeIndexInternal - 1);
                                    if (prev != null && !Double.IsNaN(prev.Value))
                                    {
                                        retVal = prev;
                                    }
                                }
                                
                                if (Double.IsNaN(retVal.Value) && inputDataSet.ContainsSignal(inputId))
                                {
        ...
        public bool Simulate (TimeSeriesDataSet inputData, bool doDetermineIndicesToIgnore, out TimeSeriesDataSet simData)
        {
            ...
            Console.WriteLine($"[PlantSimulator] Total models passed in: {modelDict.Count}, Total sorted by Topology: {orderedSimulatorIDs.Count}");
            ...
            if (!didInit)
            {
                ...
                // return false; // FORCE CONTINUE for partial results
            }
            ...
            for (int timeIdx = 0; timeIdx < N; timeIdx++)
            {
                ...
                for (int modelIdx = 0; modelIdx < orderedSimulatorIDs.Count; modelIdx++)
                {
                    ...
                    double[] outputVal = model.Iterate(inputVals, timeBase_s);
                      if (Double.IsNaN(outputVal[0]) && timeIdx < 5)
                      {
                          Console.WriteLine($"[DEBUG] Output NaN! timeIdx:{timeIdx} model:{model.GetID()} inputVals:[{String.Join(",", inputVals)}]");
                      }
